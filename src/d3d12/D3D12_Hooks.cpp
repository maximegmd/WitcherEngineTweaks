...
HRESULT D3D12::PresentDownlevel(
    ID3D12CommandQueueDownlevel* apCommandQueueDownlevel, ID3D12GraphicsCommandList* apOpenCommandList, ID3D12Resource* apSourWETex2D, HWND ahWindow,
    D3D12_DOWNLEVEL_PRESENT_FLAGS aFlags)
{
    if (WET::Get().GetOptions().Patches.DisableWin7Vsync)
        aFlags &= ~D3D12_DOWNLEVEL_PRESENT_FLAG_WAIT_FOR_VBLANK;

    auto& d3d12 = WET::Get().GetD3D12();
    ...
    auto it = std::find_if(cbegin, d3d12.m_downlevelBackbuffers.cend(), [apSourWETex2D](const auto& downlevelBackbuffer) { return downlevelBackbuffer.Get() == apSourWETex2D; });
    if (it == d3d12.m_downlevelBackbuffers.cend())
    {
        ...
        d3d12.m_downlevelBackbuffers.emplace_back(apSourWETex2D);
    ...
    if (d3d12.InitializeDownlevel(d3d12.m_pCommandQueue.Get(), apSourWETex2D, ahWindow))
        d3d12.Update();

    return d3d12.m_realPresentD3D12Downlevel(apCommandQueueDownlevel, apOpenCommandList, apSourWETex2D, ahWindow, aFlags);
}
...
void* D3D12::CRenderNode_Present_InternalPresent(int32_t* apDeviceIndex, uint8_t aSomeSync, UINT aSyncInterval)
{
    ...
    else
    {
        ...
        {
            d3d12.m_pCommandQueue = pDirectQueue;
