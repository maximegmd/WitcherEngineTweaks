...
VKBind s_overlayToggleBind{
    ...
    []
    {
        if (!WET::Get().GetBindings().IsRecordingBind())
            WET::Get().GetOverlay().Toggle();
    }};
VKBindInfo s_overlayToggleBindInfo{s_overlayToggleBind, 0, 0, false};
VKModBind s_overlayToggleModBind{"WET", s_overlayToggleBind.ID};
} // namespace
...
bool Bindings::IsFirstTimeSetup()
{
    ...
    {
        s_overlayToggleBindInfo.CodeBind = WET::Get().GetBindings().GetBindCodeForModBind(s_overlayToggleModBind);
...
bool Bindings::FirstTimeSetup()
{
    ...
    ImGui::OpenPopup("WET First Time Setup");

    if (ImGui::BeginPopupModal("WET First Time Setup", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        auto& [WETBinds, WETHotkeys] = m_vkBindInfos.at(s_overlayToggleModBind.ModName);
        UpdateAndDrawModBindings(s_overlayToggleModBind.ModName, WETBinds, WETHotkeys, true);

        auto& WETOverlayToggle = WETBinds[0];
        if (WETOverlayToggle.CodeBind != 0)
        {
            WETOverlayToggle.SavedCodeBind = WETOverlayToggle.CodeBind;
            m_bindings.Save();
    ...
    if (aVKBindInfo.IsBinding && !isRecording)
    {
        ...
        if (codeBind != 0 && codeBind != previousCodeBind)
        {
            if (m_bindings.IsFirstKeyUsed(codeBind))
            {
                ...
                {
                    const auto WETBind = modBind == s_overlayToggleModBind;
                    if (!WETBind || aVKBindInfo.Bind.IsHotkey())
                    {
                        ...
                        if (bindIt != bindInfos.cend())
                        {
                            ...
                            if (!WETBind || !bindItCodeBindIsSame)
                            {
                                const auto bindItCodeBind = bindIt->CodeBind;
                                if ((!WETBind && bindItCodeBindIsSame) || aVKBindInfo.Bind.IsInput() || bindIt->Bind.IsInput())
                                {
